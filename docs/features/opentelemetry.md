...
|--------|-------------|---------|
| `endpoint` | OTLP collector endpoint (gRPC) | Required |
| `service_name` | Service name in traces | Crate name |
    ...
    spec:
      ...
      - name: app
        ...
        - name: OTEL_RESOURCE_ATTRIBUTES
          value: "deployment.environment=production,service.namespace=backend"
```

## Viewing Traces

Once configured, you can view traces in your collector's UI:

1. **Jaeger**: http://localhost:16686
2. **Grafana/Tempo**: http://localhost:3001 (add Tempo as datasource)
3. **AWS X-Ray**: AWS Console → X-Ray → Traces
...
Traces are correlated with logs via the request ID:
